...
export async function fetchSteamSpyDataForAppIds(
	...
	const finalOptions: FetchSteamSpyDataOptions = {
		...FetchSteamSpyDataDefaultOptions,
		...options,
	};
	...
	for (const gameInfo of gameInfos) {
		...
		if (gameInfo.spy_update_timestamp && finalOptions.useCache) {
			logger.debug(
				"skipping SteamSpy fetch for %s as data already exists",
				gameInfo.name,
			);
		...
		try {
			const response = await fetch(
				`${STEAM_SPY_API_ENDPOINT}?request=appdetails&appid=${gameInfo.appId}`,
			);
			...
			if (data && data.name) {
				...
				logger.warn(
					"no SteamSpy data found for App ID %d (%s)",
					gameInfo.appId,
					gameInfo.name,
				);
			}
			...
			logger.error(
				"failed to fetch SteamSpy data for App ID %d (%s): %s",
				gameInfo.appId,
				gameInfo.name,
				err,
			);
 ...
 */
function determineReviewCategory(
	...
	if (
		totalReviews >= REVIEWS_OVERWHELMINGLY_THRESHOLD &&
