...
import type { ProcessedSteamGameInfo } from "./steam/types";
...
if (DIRECTUS_API_ENDPOINT && DIRECTUS_API_TOKEN) {
	createDirectus(DIRECTUS_API_ENDPOINT)
		.with(staticToken(DIRECTUS_API_TOKEN))
		.with(rest());
}

export async function upsertAllSteamGames(
	steamGameData: ProcessedSteamGameInfo[],
	logger: Logger,
) {
	...
	if (!DIRECTUS_CLIENT) {
		logger.error(
			"Directus API endpoint or token not configured; skipping Directus sync",
		);
	...
	const steamAppIdToDirectusItemIdMap =
		createSteamAppIdToDirectusItemIdMap(existingGames);
	...
	for (const gameData of steamGameData) {
		...
		logProgress(
			steamGameData.indexOf(gameData) + 1,
			steamGameData.length,
			"Directus upsert",
			logger,
		);
		...
		const data = {
			...
			[DIRECTUS_GAME_SCREENSHOTS_KEY]: gameData.screenshots
			...
			[DIRECTUS_GAME_STEAM_POSITIVE_REVIEWS_KEY]:
				gameData.total_positive_reviews || 0,
			[DIRECTUS_GAME_STEAM_NEGATIVE_REVIEWS_KEY]:
				gameData.total_negative_reviews || 0,
			[DIRECTUS_GAME_LAST_PLAYED_KEY]: gameData.basicData?.isAdmin
				? gameData.basicData?.lastPlayed || null
				: null,
			[DIRECTUS_GAME_HOURS_KEY]: gameData.basicData?.isAdmin
				? gameData.basicData?.hours || 0
				: 0,
			[DIRECTUS_GAME_HLTB_HOURS_KEY]: gameData.hltb_hours || null,
			[DIRECTUS_GAME_HLTB_HOURS_EXTRA_KEY]: gameData.hltb_hours_extra || null,
			[DIRECTUS_GAME_HLTB_HOURS_COMPLETIONIST_KEY]:
				gameData.hltb_hours_completionist || null,
			...
			[DIRECTUS_GAME_RELEASE_DATE_KEY]: gameData.release_date_timestamp
				...
				: null,
			[DIRECTUS_GAME_STEAM_SPY_AVERAGE_FOREVER_KEY]:
				gameData.spy_average_forever || 0,
			[DIRECTUS_GAME_STEAM_SPY_AVERAGE_2_WEEKS_KEY]:
				gameData.spy_average_2weeks || 0,
			[DIRECTUS_GAME_STEAM_SPY_MEDIAN_FOREVER_KEY]:
				gameData.spy_median_forever || 0,
			[DIRECTUS_GAME_STEAM_SPY_MEDIAN_2_WEEKS_KEY]:
				gameData.spy_median_2weeks || 0,
		...
		if (directusItemId) {
			try {
				await DIRECTUS_CLIENT.request(
					updateItem(DIRECTUS_GAME_COLLECTION_NAME, directusItemId, data),
				);
				...
				logger.error(
					"one failure here means others will likely fail; aborting",
				);
			...
			try {
				...
				await DIRECTUS_CLIENT.request(
					createItem(DIRECTUS_GAME_COLLECTION_NAME, data),
				);

				logger.info(
					"created Directus item for Steam App ID %d (%s)",
					gameData.appId,
					gameData.name,
				);
			} catch (err) {
...
function createSteamAppIdToDirectusItemIdMap(
	existingGames: Record<number, any>,
): Record<number, any> {
...
async function fetchExistingGames(
	...
	for (let page = 1; page <= DIRECTUS_MAX_PAGES; page++) {
		...
		if (response.length === 0) {
			logger.debug(
				"no more existing Directus games found, stopping at page %d",
				page,
			);
