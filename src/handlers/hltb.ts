...
import {
	loadDissimilarGamesCache,
	updateDissimilarGamesCache,
} from "./steam/caches";
...
export async function processHltbDataForSteamGames(
	...
	const finalOptions: FetchHLTBDataOptions = {
		...FetchHLTBDataDefaultOptions,
		...options,
	};
	...
	try {
		...
		logger.error(
			"failed to capture HLTB auth token: %s, aborting HLTB data fetch",
			err,
		);
	...
	const dissimilarGamesCache = await loadDissimilarGamesCache(
		"dissimilar",
		logger,
	);
	const unconfidentGamesCache = await loadDissimilarGamesCache(
		"unconfident",
		logger,
	);
	...
	const dissimilarNameWarnings: {
		appId: number;
		gameInfoName: string;
		matchedName: string;
	}[] = [];
	const nonConfidentMatchWarnings: {
		appId: number;
		gameInfoName: string;
		matchedName: string;
	}[] = [];
	...
	for (const gameInfo of gameInfos) {
		...
		const hasCachedSimilarityEntry =
			similarityEntry && similarityEntry.status === "yes";
		...
		const appNameContainsHltbName =
			sanitizedGameInfoName.includes(sanitizedHltbName);
		const hltbNameContainsAppName = sanitizedHltbName.includes(
			sanitizedGameInfoName,
		);

		const namesAreSimilar =
			appNameMatchesHltbName ||
			appNameContainsHltbName ||
			hltbNameContainsAppName;
		...
		if (isExplicitlyDissimilar) {
			logger.warn(
				"going ahead with HLTB fetch for %s because of explicit dissimilarity",
				gameInfo.name,
			);
		} else if (gameInfo.last_hltb_update_timestamp && finalOptions.useCache) {
			logger.debug(
				"skipping HLTB fetch for %s as data already exists",
				gameInfo.name,
			);
		...
		try {
			...
			if (result && result.length > 0) {
				...
				gameInfo.hltb_hours_completionist = Math.round(
					hltbData.comp_100 / 60 / 60,
				);
				gameInfo.hltb_url = `https://howlongtobeat.com/game/${hltbData.game_id}`;
			} else {
				logger.debug(
					"no HLTB data found for %s but request succeeded",
					gameInfo.name,
				);
			}
			...
			if (failedAttempts >= MAX_HLTB_FAILED_ATTEMPTS) {
				logger.error(
					"maximum HLTB failed attempts reached; aborting HLTB data fetch",
				);
			...
			try {
				...
				logger.error(
					"failed to recapture HLTB auth token: %s, aborting...",
					err,
				);
	...
	await updateDissimilarGamesCache(
		"dissimilar",
		dissimilarGamesCache,
		dissimilarNameWarnings,
		logger,
	);
	await updateDissimilarGamesCache(
		"unconfident",
		unconfidentGamesCache,
		nonConfidentMatchWarnings,
		logger,
	);
...
async function makeHtlbSearchRequest(
	query: string,
	authToken: string,
): Promise<any> {
