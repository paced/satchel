...
const GAME_KNOWN_DELETED_CACHE_PATH =
	"out/steam-known-deleted-games-cache.json";
const GAME_DISSIMILAR_GAMES_CACHE_PATH = "out/steam-dissimilar-games-cache.txt";
const GAME_UNCONFIDENT_GAMES_CACHE_PATH =
	"out/steam-unconfident-games-cache.txt";

export async function loadGameInfoCache(
	logger: Logger,
...
export async function updateSteamGameInfoCache(
	...
	try {
		...
		const newGameInfosKeys = new Set(
			gameInfos.map((gameInfo) => gameInfo.appId),
		);
		...
		const uniqueGameInfos = Object.values(combinedGameInfos).sort(
			(a, b) => a.appId - b.appId,
		);
		...
		await writeFile(
			GAME_INFO_CACHE_PATH,
			JSON.stringify(gameInfosWithoutPersonalization, null, 2),
			"utf-8",
		);

		logger.info(
			"wrote %d games to cache",
			gameInfosWithoutPersonalization.length,
		);
	} catch (err) {
...
export async function loadOwnedGamesCache(
	...
	try {
		...
		logger.info(
			"loaded %d owned Steam app IDs from cache",
			cachedAppIds.length,
		);
	} catch (err) {
...
export async function updateOwnedGamesCache(
	...
	try {
		...
		await writeFile(
			cacheFilename,
			JSON.stringify(sortedAppIds, null, 2),
			"utf-8",
		);

		logger.info(
			"wrote %d owned app IDs to owned game cache",
			sortedAppIds.length,
		);
	} catch (err) {
...
export async function loadKnownDeletedGamesCache(
	logger: Logger,
): Promise<number[]> {
	...
	try {
		...
		logger.info(
			"loaded %d known deleted Steam app IDs from cache",
			knownDeletedAppIds.length,
		);
	} catch (err) {
		logger.info(
			"no known deleted games cache file found, will create new cache",
		);
...
export async function updateKnownDeletedGamesCache(
	appIds: number[],
	logger: Logger,
): Promise<void> {
	try {
...
> {
	const cacheFilename =
		mode === "dissimilar"
			? GAME_DISSIMILAR_GAMES_CACHE_PATH
			: GAME_UNCONFIDENT_GAMES_CACHE_PATH;
	...
	try {
		...
		lines.forEach((line) => {
			const match = line.match(
				/^\((\d+)\) Is (.+) really (.+)\? \[(yes|no|unconfirmed)]/,
			);
			if (match) {
		...
		logger.info(
			"loaded %d dissimilar/unconfident games from cache",
			Object.keys(dissimilarGamesMap).length,
		);
	} catch (err) {
		logger.info(
			"no dissimilar/unconfident games cache file found, will create new cache",
		);
	}
...
export async function updateDissimilarGamesCache(
	...
	const sortedEntries = Array.from(mergedGamesMap.entries()).sort(
		(a, b) => a[0] - b[0],
	);
