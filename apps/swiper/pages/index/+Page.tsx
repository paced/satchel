...
export default function Home() {
  ...
  const steamReviewsText =
    currentGame?.Steam_Positive_Reviews && currentGame?.Steam_Negative_Reviews
      ? `(${currentGame.Review_Category}) ( ${Math.round((100.0 * currentGame.Steam_Positive_Reviews) / currentGame.Steam_Total_Reviews)}%)`
      : "N/A";
        ...
        >
          {(currentGame.Screenshots?.split("\n") || []).map((screenshotUrl: string, index: number) => (
            <CarouselSlide key={index}>
              <Image src={screenshotUrl} style={{ maxHeight: "384px", objectFit: "contain" }} />
            </CarouselSlide>
          ))}
        </Carousel>
      </Box>
      <Grid mt="md">
        <GridCol span={{ base: 12, md: 6 }} h="100%">
          ...
          <Box my="md" style={{ textAlign: "center" }}>
            {currentGame?.Tags.filter((tag: any) => !TAGS_TO_REMOVE.includes(tag)).map((tag: string) => (
      ...
      <Grid my="md">
        ...
        <GridCol span={{ base: 12, md: 4 }}>
          <Flex direction="row" align="center" justify="center" mx="auto">
            <Flex direction="column" align="center">
              <Title order={4}>{currentGame?.HLTB_Hours || "N/A"}</Title>
              <Text size="xs">HLTB Main</Text>
            </Flex>
            <Flex direction="column" ml="lg" align="center">
              <Title order={4}>{currentGame?.HLTB_Extra || "N/A"}</Title>
              <Text size="xs">HLTB Extra</Text>
            </Flex>
            <Flex direction="column" ml="lg" align="center">
              <Title order={4}>{currentGame?.HLTB_Completionist || "N/A"}</Title>
        ...
        <GridCol span={{ base: 12, md: 4 }}>
          <Flex direction="row" align="center" justify="center" mx="auto">
            <Flex direction="column" ml="lg" align="center">
              <Title order={4}>{currentGame?.Hours || "N/A"}</Title>
              <Text size="xs">Your Hours</Text>
            </Flex>
            <Flex direction="column" ml="lg" align="center">
              <Title order={4}>{currentGame?.Last_Played?.toLocaleString() || "N/A"}</Title>
              <Text size="xs">Last Played</Text>
            </Flex>
          </Flex>
        </GridCol>
      </Grid>
